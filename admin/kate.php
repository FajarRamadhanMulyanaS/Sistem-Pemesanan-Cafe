...
if ($_SESSION['level'] == ""){
 ...
 if (isset($_POST['simpan'])) {
  ...
  if(isset($_GET['edit'])){
   ...
   if(isset($_GET['hapus'])){
		$sql = mysqli_query($koneksi,"DELETE FROM tb_kategori WHERE kd_kategori = '$_GET[id]'");

		echo "<script>alert('Data Terhapus');document.location.href='?menu=kategori'</script>";
	}
	if (isset($_POST['updateuser'])) {
		$sql = mysqli_query($koneksi,"UPDATE tb_kategori SET kd_kategori='$_POST[kd_kategori]',kategori='$_POST[kategori]' WHERE kd_kategori='$_GET[id]'");
		if($sql){
